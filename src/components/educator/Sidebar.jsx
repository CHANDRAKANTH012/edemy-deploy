...
const Sidebar = () => {
  ...
  return (
    isEducator && (
      <div className="edemy-sidebar-wrapper">
        {menuItems.map((item) => (
          <NavLink
            to={item.path}
            key={item.name}
            ...
            className={({ isActive }) =>
              `edemy-sidebar-nav-link ${isActive ? "active" : ""}`
            }
          >
            <img
              src={item.icon}
              alt="icon"
              className="edemy-sidebar-nav-icon"
            />
            <p className="edemy-sidebar-nav-text">{item.name}</p>
          </NavLink>
        ))}
      </div>
    )
